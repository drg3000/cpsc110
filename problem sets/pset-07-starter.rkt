...
;;produces a string from first list of letters given, based on operations list

(check-expect (decoder empty empty) "")

(check-expect (decoder (list "i" "k" "l" "h" "a" "m" "k" "s" "a" "r" "m")
                       (list "keep" "remove" "remove" "space"
                             "keep" "keep" "space" "keep" "keep" "remove"))
              "i am sam")
...
(check-expect (decoder empty (list "keep" "space")) "")

(check-expect (decoder (list "b" "i" "n" "g") (cons "keep" empty))
              "bing")
...
-------------------------|-------------|-------------------------------------|
                  lst1   |     empty   |                                     |
   ops                   |             | cons 1String ListOf1String          |
-------------------------|-------------|-------------------------------------|
                 empty   |       ""    |(foldl string-append "" lst1)        |
                         |             |                                 [2] |
-------------------------|             |-------------------------------------|
cons "keep" Operations   |       ""    |(cond [(string=? (first ops) "keep") |
                         |             |(string-append (first lst1)          |
                         |             |(decoder (rest lst1) (rest ops)))]   |
...
cons "space" Operations  |       ""    |                                     |
                         |             |                                     | 
cons "remove" Operations |       "" [1]|                                 [3] |
-------------------------|-------------|-------------------------------------|
|#

;(define (decoder lst1 ops) "");stub

(define (decoder l ops)
  (cond [(empty? l) ""]; [1]
        [(empty? ops) (foldr string-append "" l)]; [2]
        [else ; [3]
         (cond [(string=? (first ops) "keep")
...
;; CONSTRAINT: for max-course-num--loc (encapsulated)
;; given list has at least 1 element
...
(check-expect (max-course-num C213) 317)
...
(@signature Course -> ListOfNatural)
;;produces numbers of all courses that:
;; - have odd course number
;; - are not pre-reqs for any other courses in the tree
;; - are 3 credits or more

(check-expect (three-criteria-courses C100) empty)
(check-expect (three-criteria-courses C310) (list 319))
(check-expect (three-criteria-courses C210) (list 313 317 313 317 319 311))
(check-expect (three-criteria-courses C110)
              (list 203 313 317 313 317 319 311 303))

;(define (three-criteria-courses c) empty);stub

(@template-origin Course ListOfCourse encapsulated)

(define (three-criteria-courses c)
  (local [(define (three-crit-c--course c) 
            (if (and (odd? (course-number c))
                     (>= (course-credits c) 3) (empty? (course-dependents c)))
                (cons (course-number c) empty)
                (three-crit-c--loc (course-dependents c))))            

          (define (three-crit-c--loc loc)
            (cond [(empty? loc) empty]
                  [else
                   (append (three-crit-c--course (first loc))
                           (three-crit-c--loc(rest loc)))]))]

    (three-crit-c--course c)))
